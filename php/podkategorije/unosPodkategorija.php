...
if (!empty($_POST)) {
    ...
    if ($valid) {
        ...
        try {
            ...
            $query = $pdo->prepare(
                'INSERT INTO
                    TechnoShop.Podkategorija  (podkategorija_naziv)
                VALUES
                    (:naziv);'
            );
...
<div class="container">
    <h3>
        Forma za unos novih podkategorija
        <small>
            Ovde možete uneti nove podkategorije ili izbrisati postojeće.
        </small>
    </h3>
    ...
    <p>
        <a class="btn btn-primary"
           ...
           aria-controls="collapseExample">
            Forma za unos podkategorija
        </a>
    </p>
    <div class="collapse" id="collapseExample">
        <div class="container">
            <div class="forma">
                <form action="unosPodkategorija.php" method="post">
                    <hr>
                    <div class="control-group <?php echo !empty($nazivError) ? 'error' : ''; ?>">
                        <div class="controls">
                            <input class="form-control" name="naziv" type="text" placeholder="Naziv podkategorije">
...
<div class="container">
    <div class="row">
        ...
        try {
            ...
            $query = $pdo->prepare(
                'SELECT * FROM TechnoShop.Podkategorija'
            );
            ...
            while ($row = $query->fetch()) {
                ...
                <div class="card text-white bg-dark" style="box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);">
                    <div class="card-body">
                        ...
                        <a href="ukloniPodkategoriju.php?id=' . $row['podkategorija_id'] . '">
                            <button class="btn btn-danger">Ukloni podkategoriju</button>
                        </a>
                    </div>
                </div>
            </div>';
